...
- **表格/结构化数据**：特征工程之后的分类回归（风控评分、销量预测），MLP 至今是工业界强基线
- **推荐系统**：Embedding + MLP 的 CTR 模型是广告/推荐标配骨架——"稀疏特征查表 + 全连接"就是 MLP 的变形
- **一切网络的"积木"**：CNN 的分类头、Transformer 的 FFN 子层本质都是 MLP——学它不是学一个模型，是认领一块通用砖
