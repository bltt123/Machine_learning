...
- **大模型训练现场**：训 GPT/BERT 默认用 AdamW 而非 SGD——本项目 SGD lr 1e-3 只有 58% 换 0.1 追到 95.75% 就是原因，lr 不对优化器背锅；warmup+cosine、梯度裁剪/累积、混合精度都在这套工具箱延长线上
- **部署前的早停**：线上训一天的任务 val 两轮不涨就刹车，本项目 patience=2 第 9 轮止步 96.91% 即省几千块 GPU 钱，早停本身就是正则
- **推荐/广告的校准**：把"100%是猫"软化成"90%是猫"的 Label Smoothing 让模型别太独断，支撑：小 lr 下 96.90% 意外最大增益；思想广泛用于知识蒸馏与 CTR 概率校准
- **工业消融文化**：上一个 trick 值不值——按本项目"锚点+单变量+固定种子"做 A/B，dead ReLU 42.3% 这类 hook 统计、BN batch=2 暴跌 21.6pt 这类对照就是证据
