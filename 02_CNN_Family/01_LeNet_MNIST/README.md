...
- **历史现场**：1998 年美国邮政手写邮编、银行支票自动识别——CNN 第一次大规模商用，LeCun 用它证明了"端到端训练"可行
- **今天的小场景**：MNIST 级别的简单 OCR（仪表读数、表单数字、验证码）；6 万参数小到手机 CPU 甚至单片机都能毫秒级推理
- **思想遗产**："卷积→池化→卷积→池化→全连接"的三明治骨架，是一切现代视觉网络的底座
