...
- **ResNet 家族**：今天做图像分类/目标检测/分割，骨干默认选 ResNet-50（在 COCO 检测上 Faster R-CNN 的 backbone 就是它）；Transformer 每个块也抄了这条跳跃连接——"加一条线"是深度学习性价比最高的发明，支撑：本项目 32 层残差增益 +15.4pt、Plain 退化 -9.25pt 复现
- **残差 + 归一化的组合拳**：GPT/ViT 全是"残差 + LayerNorm"堆出来的摩天楼——本章"残差保梯度 + BN 稳分布（+10.62pt）"两条腿的现代变体
- **消融方法论**：开关式消融（一个类 + `residual/use_bn` 两个布尔）是论文复现与工业 A/B 的标准姿势——本项目五臂同配方 15 epochs 就是模板
