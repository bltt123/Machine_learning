...
- **DenseNet**：小数据/医疗影像等"参数效率"优先场景；但大分辨率下显存开销使其少于 ResNet 成为 backbone 默认
- **Inception**：多尺度思想进入后续所有架构（FPN 特征金字塔的雏形）；1×1 降维成为 bottleneck 标配（ResNet-bottleneck、MobileNet 均继承）
- **拓扑选择**：相加（省显存）vs 拼接（省参数）vs 并行（多尺度），本章三臂同台让选择有了数字依据
