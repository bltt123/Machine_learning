...
- **MobileNet**：手机端要实时——拍照分类/扫一扫/相册人脸聚类、端侧检测的 backbone 默认起点；支撑：本项目分解 7.9×、α=0.5 以 27% 参数 29% 计算反超 α=1 +0.64pt，小数据小模型更不背题
- **EfficientNet**：云端要"算力花在刀刃上"——给同样算力它把深度/宽度/分辨率按比例一起放大，帕累托前沿标杆；思想进入后续所有 NAS 搜索空间，支撑：本项目 宽×2+深×2 用 40M MACs 换 58.66%（0.20pt/10M 递减）
- **SE 注意力**：从 04 选学到 05 标配——给每个通道学一个权重"重要就放大"，成为现代 CNN/Transformer 通道注意力的通用件，MobileNet/EfficientNet 全内置
